...
fp = open('75.txt', 'r')
file_text = fp.read()
digi_str = re.findall(r'([0-9]+)',file_text,re.MULTILINE)#正则表达式
fp.close()
coordinates = []
...
for i in range(75):
    city_location.append((coordinates[i][0],coordinates[i][1]))
    city_num.append(i+1)
...
def route_mile_cost(route):     #计算路径的里程
    mile_cost = 0.0
...
def random_initial_route(remain_cities):    #随机生成初始路径
...
def improve_circle(remain_cities):  #改良圈算法生成初始路径
    initial_route = remain_cities[:]
    random.shuffle(initial_route)
...
def nearest_city(current_city,cand_cities):#获取当前邻居城市中距离最短的1个
    temp_min = float('inf')
    next_city = None
...
def random_swap_2_city(route):  #随机选取两个城市并将这两个城市之间的点倒置,计算其里程成本
    new_route = route[:]
...
def generate_new_route(route):  #生成新的路线
...
def tabu_search(remain_cities,iteration_count=100):
    global tabu_list,best_so_far_cost,best_so_far_route
    #生成初始解
    best_so_far_route,best_so_far_cost = improve_circle(remain_cities)#选择生成初始路径的方法
...
if __name__ == '__main__':
    ...
    for j in range(M):
        ...
        AM = sum(Mcosts)/(j+1) #平均里程
        AT = sum(Tcosts)/(j+1) #平均时间
